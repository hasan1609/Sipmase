package com.sipmase.sipmase.admin.seawater.fragment
...
import com.sipmase.sipmase.admin.seawater.activity.DetailCekSeaWaterActivity
import com.sipmase.sipmase.databinding.FragmentHasilSeawaterBinding
import com.sipmase.sipmase.databinding.FragmentScheduleSeawaterBinding
...
import com.sipmat.sipmat.adapter.damkar.SchedleSeaWaterAdapter
...
class HasilSeaWaterFragment : Fragment(), AnkoLogger {
    lateinit var binding: FragmentHasilSeawaterBinding
    var api = ApiClient.instance()
    lateinit var progressDialog: ProgressDialog
    var tahun: String? = null
    private lateinit var mAdapter: SchedleSeaWaterAdapter
    var tw: String? = null
    val sheet = BottomSheetAddScheduleSeaWaterFragment()
    ...
    override fun onCreateView(
        ...
        binding =
            DataBindingUtil.inflate(inflater, R.layout.fragment_hasil_seawater, container, false)
        ...
        binding.rbtw.setOnCheckedChangeListener { group, checkedId ->
            when (checkedId) {
                ...
                else -> binding.rvscheduleseawater.visibility = View.GONE
            }
        }
        return binding.root
    }
    ...
    fun getschedule(tw: String, tahun: String) {
        binding.rvscheduleseawater.layoutManager = LinearLayoutManager(requireActivity())
                    ...
                    try {
                        if (response.isSuccessful) {
                            ...
                            if (data!!.data!!.isEmpty()) {
                                ...
                                binding.rvscheduleseawater.visibility = View.GONE
                                toast("Data Hasil Masih Kosong")
                            } else {
                                loading(false)
                                binding.tvkosong.visibility = View.GONE
                                binding.rvscheduleseawater.visibility = View.VISIBLE
                                for (hasil in data.data!!) {
                                    notesList.add(hasil)
                                    mAdapter = SchedleSeaWaterAdapter(notesList, requireActivity())
                                    binding.rvscheduleseawater.adapter = mAdapter
                                    mAdapter.setDialog(object : SchedleSeaWaterAdapter.Dialog {
