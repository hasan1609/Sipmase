...
class BottomSheetAddScheduleAparFragment : BottomSheetDialogFragment(), AnkoLogger {
    ...
    lateinit var itemAparScheduleAdapter: ItemAparScheduleAdapter
    ...
    override fun onCreateView(
        ...
        binding.txtJudul.text = "Schedule APAR"
        ...
        binding.searchBar.setOnQueryTextListener(object : SearchView.OnQueryTextListener,
            ...
            override fun onQueryTextSubmit(query: String?): Boolean {
                itemAparScheduleAdapter.getFilter().filter(query)
                return true
            }

            override fun onQueryTextChange(newText: String?): Boolean {
                itemAparScheduleAdapter.getFilter().filter(newText)
                return true
            }
        ...
        binding.btnsubmit.setOnClickListener {
            sb = StringBuilder()
            var i = 0
            while (i < itemAparScheduleAdapter.lista.size) {
                val spiritualTeacher = itemAparScheduleAdapter.lista[i]
                sb!!.append(spiritualTeacher.kode)
                if (i != itemAparScheduleAdapter.lista.size - 1) {
                    sb!!.append("n")
                }
                i++
            ...
            if (selectedRadioButtonId != -1 && tanggal_cek!=null && itemAparScheduleAdapter.lista.size > 0 && tahun.isNotEmpty()){
                loading(true)
                selectedRadioButton = find(selectedRadioButtonId)
                val tw: String = selectedRadioButton.text.toString()
                api.schedule_apar(sb.toString(),tw,tahun,tanggal_cek!!).enqueue(object :Callback<PostDataResponse>{
            ...
            override fun onResponse(
                ...
                if (response.isSuccessful) {
                    ...
                    for (hasil in data!!.data!!) {
                        notesList.add(hasil)
                        itemAparScheduleAdapter = ItemAparScheduleAdapter(notesList, requireActivity())
                        binding.rvItem.adapter = itemAparScheduleAdapter
                        itemAparScheduleAdapter.notifyDataSetChanged()
                    }
                }
            }
        }
